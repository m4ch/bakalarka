<?php
	...
	$json["type"] = "new_models_of_model_category_n_concurency";
	$json["model"] = "Škoda Fabia";
	$json["year_from"] = 2008;
	$json["month_from"] = 1;
	$json["year_to"] = 2009;
	$json["month_to"] = 1;
	...
	$json["type"] = "new_models_of_model_fuels";
	$res = $db->query("CALL selectNewBrands('".json_encode($json,JSON_UNESCAPED_UNICODE)."')");
	echo $db->error;
	foreach($res as $item){
...
<div style="background-image: url(konkretni_model.jpg);  background-repeat: no-repeat;
  background-size: auto;position: relative;width: 100%;height:3600px; margin: 0 auto;">
<div style="position: absolute;width:530px;height:275px;top: 2575px;left:400px;">
<canvas id="newBrandsChart"></canvas>
</div>
<div style="position: absolute;width:530px;height:275px;top: 2575px;left:1000px;">
...
var chart = new Chart(ctx, {
    ...
    options: {
		...
		title: { 
			display: true, 
			text: 'Top 5 nejprodavanejsi modely v kategorii',
			fontSize: 21,
...
var chart2 = new Chart(ctx2, {
    ...
    options: {
		...
		title: { 
			display: true, 
			text: 'Top 5 nejprodavanejsi paliva',
			fontSize: 21,
			fontFamily: 'Nunito Sans'},
		tooltips: {
			bodyFontSize: 17,
